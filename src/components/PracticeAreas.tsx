        ...
        <div ref={desktopContainerRef} className="hidden md:block relative min-h-[520px]">
          ...
          >
            {bottomRowAreas.map((area, idx) => (
              <div
                key={area.id}
                className="h-full p-6 sm:p-8 rounded-2xl bg-[var(--bg-card)] border-2 border-[#0B192C]/30 dark:border-[#D1D5DB]/40 shadow-[0_10px_30px_rgba(11,25,44,0.12)] dark:shadow-[0_10px_30px_rgba(0,0,0,0.4)] flex flex-col justify-between group transition-all duration-300"
              >
                <div>
                  <div className="flex items-center justify-between mb-4">
                    <span className="font-heading text-2xl font-bold text-[var(--accent)]">
